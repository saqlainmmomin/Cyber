...
AI-powered multi-framework compliance maturity platform (DPDPA, ISO 27001, GDPR, HIPAA, NIST CSF, PCI-DSS) with a Jinja2 + HTMX + Tailwind web portal. Desk-review pre-fill, adaptive tiering, domain screening, two-call Codex gap analysis, deterministic scoring, board-ready PDF/RFI reports — controls de-duplicated across frameworks via Unified Control Clusters.
...
| `app/services/claude_analyzer.py` | Evidence extraction + gap analysis Codex calls |
| `app/services/scoring.py` | Deterministic scoring engine (never Codex) |
...
- **Scoring is deterministic, server-side** — Codex outputs qualitative strings only.

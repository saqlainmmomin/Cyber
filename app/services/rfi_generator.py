...
Structure:
  ...
  - Introduction (purpose, framework context)
  - Evidence Items (grouped by framework chapter/domain):
      Item ID, Description, Requirement Reference,
...
def generate_rfi(
    ...
    framework_names are the display names of the frameworks assessed
    (e.g. ["India DPDPA", "ISO 27001"]) — used in the document title and
    Claude prose prompts.

    Returns a dict with:
      title, introduction, evidence_items (list), response_instructions, appendix,
      total_items, critical_items, raw_ai_response
    """
    frameworks_label = ", ".join(framework_names) if framework_names else ""
    title_prefix = f"{frameworks_label} Compliance" if frameworks_label else "Compliance"
    ...
    if not evidence_items:
        return {
            "title": f"{title_prefix} — Request for Information: {company_name}",
    ...
    raw_response = _call_claude_rfi(company_name, industry, evidence_items, frameworks_label)
...
def _build_evidence_items(
    ...
    for gap in gap_items:
        ...
        items.append({
            ...
            "section_ref": _REQ_SECTIONS.get(req_id, ""),
...
def _call_claude_rfi(company_name: str, industry: str, items: list[dict], frameworks_label: str = "") -> dict:
    ...
    prompt = f"""Generate a professional Request for Information (RFI) document for {compliance_context}.
...
1. **introduction**: A professional 2-3 paragraph introduction explaining:
   - This RFI is part of a {compliance_context} gap assessment
...
"""
    ...
    message = client.messages.create(
        ...
        temperature=0.2,
        system=(
            f"You are a senior compliance consultant ({frameworks_label or 'multi-framework'}) drafting a formal Request for Information. "
...
def _merge_claude_enhancements(base_items: list[dict], claude_response: dict) -> dict:
    ...
    for item in base_items:
        ...
        if claude_item.get("evidence_requested"):
            ...
            item["evidence_requested"] = f"Please provide documentation demonstrating compliance with {item['requirement_title']} ({item['section_ref']})."
...
def _default_introduction(company_name: str, date: str) -> str:
    return (
        f"This Request for Information (RFI) has been prepared as part of the compliance gap assessment "
        f"for {company_name}. The assessment has identified areas where additional evidence "

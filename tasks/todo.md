...
- [x] **P3-4: Remediation tracking** — implemented and verified: 12 contract scenarios cover evidence-backed close, separate verification, re-open, derived Finding status, legacy retirement, report Action counts, engagement rollup and structural guards; 489 full-suite tests pass (477 baseline + 12 new). ASGI smoke covered the full Action lifecycle, SQL history/status output, tracker/report routes and removal of the legacy PATCH. Sonnet adversarial review found no blocking issues (closure/verification separation and legacy non-grandfathering both independently re-verified). Handoff Results: `tasks/handoffs/2026-09-23-p3-4-remediation-tracking.md#results`. **PR #33, open, not yet merged.**

**Phase 3 exit:** ✅ append-only Findings/Actions with evidence-backed closure and verification, write-once immutable report snapshots, per-framework PDF scores with citations and no blend, an engagement-level integrated report, and a fully retired legacy remediation path. Full suite: 490 passed.

...
Extended `test_no_template_reads_retired_score` to scan retired-column `overall_score` attribute/keyword accesses across every Python file under `app/`, enforce the handoff allow-list, and AST-check every exact `analysis.py` token for the permitted writes and per-framework comprehensions. Legitimate per-framework schema fields and dictionary-key reads are not retired-column accesses. The guard passes against the real codebase with no false-positive suppression; the full suite remains green at `249 passed`.

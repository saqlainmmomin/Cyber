...
**Update 2026-09-25 (after live smoke):** P6-1c (PR #56) raises the framework-path ceiling to a configurable `llm_max_output_tokens_framework` setting (default 65536). Live smoke on ISO + `c0-example` showed desk review produced 24,591 tokens in 1,032 s (`stop`), which would have been truncated under the old 16,384 cap. The single ISO judge call dropped with `APIConnectionError` after 354 s, indicating that single long calls are unreliable (~24 tokens/s, so a full ISO judge answer would be ~20 minutes in one stream). Therefore, batching (P6-1b) is required for reliability and latency, not only for the token limit; P6-1c is headroom. If P6-1c has merged first, per-batch `max_tokens` should be `min(settings.llm_max_output_tokens_framework, <your per-batch value>)` or the per-batch value chosen here — state which in Results; do not reintroduce a hard-coded ceiling above the setting.
